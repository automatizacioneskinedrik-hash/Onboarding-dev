 ...
 * JWT token verification using repository-backed persistence.
 */

const { createLogger } = require('../../services/observability/logger');
const { setRequestContext } = require('../../services/observability/request-context.service');
const { getAppContainer } = require('../../composition-root');
const { generateToken, getJwtSecret, FALLBACK_JWT_SECRET } = require('../../services/auth/jwt.service');
...
const protect = async (req, res, next) => {
    try {
        ...
        const decoded = require('jsonwebtoken').verify(token, getJwtSecretWithWarning());
        const user = await getUsersRepository().findById(decoded.id);
        ...
        req.user = getUsersRepository().safe(user);
